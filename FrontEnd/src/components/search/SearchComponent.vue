<template>
  <div class="section">
    ...
    <div>
      <div>
        <v-btn variant="outlined" class="search-btn" rounded v-for="(search, i) in searchCheckBox" :key="i"
          :active="search.isActivate" @click="checkBox(i)">
          {{ search.name }}
        </v-btn>
        <v-icon v-if="showIcon(searchCheckBox)" icon="mdi-close" style="cursor: pointer;"
      ...
      <div>
        <v-btn variant="outlined" class="search-btn" rounded v-for="(search1, i) in searchCheckBox2" :key="i"
          :active="search1.isActivate" @click="checkBox2(i)">
          {{ search1.name }}
        </v-btn>
        <v-icon v-if="showIcon(searchCheckBox2)" icon="mdi-close" style="cursor: pointer;"
          @click="resetCheckBox(searchCheckBox2)">
        </v-icon>
      </div>
      <div class="text-field-container">
        <v-text-field v-model="searchText" variant="plain" class="search-text" prepend-inner-icon="mdi-magnify"
          label="농작물 이름을 입력해 주세요." single-line>
        </v-text-field>
      </div>
      <div class="crop-container">
        <v-row>
          <v-col class="crop-col" cols="3" v-for="(crop, i) in filteredCropData" :key="i">
            <div class="image-container">
              <img referrerpolicy="no-referrer" :src="crop.thumbnailUrl" style="cursor: pointer;"
