package com.ssafy.social2.common.login.filter;
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    // "/login" + POST로 온 요청에 매칭된다. project 특성에 맞춰 변환하면 됨
    ...
    /**
     * attemptAuthentication는 인증 처리 메소드
     *
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        ...
        Map<String, String> usernamePsswordMap = objectMapper.readValue(messageBody, Map.class);

        String email = usernamePsswordMap.get(USERNAME_KEY);
        String password = usernamePsswordMap.get(PASSWORD_KEY);

        UsernamePasswordAuthenticationToken authRequest = new UsernamePasswordAuthenticationToken(email, password); //email = principal 과 password = credentials가 됨
